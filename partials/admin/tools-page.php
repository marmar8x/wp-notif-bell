...
<div class="wpnb-ad-box">
    <div class="wpnb-ad-header">
        <h3 class="wpnb-ad-header-title">
            ...
            <span class="wpnb-ad-header-tab rnd org"><?php _e('Tools', 'wp-notif-bell'); ?></span>
        </h3>

        <p class="wpnb-ad-header-text">
            <?php _e('Tools needed', 'wp-notif-bell'); ?>
        </p>
    </div>
    ...
    <div class="wpnb-w-100" style="line-height: 1.7rem;font-size: 1.0rem;">
        <h2 class="nav-tab-wrapper" style="border-bottom: 0; margin-bottom: -21px; padding-left: 10px; padding-right: 10px; ">
            <?php foreach ($_tabs as $tb => $data): ?>
                <a href="<?php echo _make_tab_url($tb); ?>" class="nav-tab<?php echo $tb === $_tab ? ' nav-tab-active' : ''; ?>"><?php echo $data['name']; ?></a>
            <?php endforeach; ?>
        </h2>
        ...
        <div class="tab-content wpnb-ad-l-box wpnb-mx-0">
            <p>
                <?php _e('This tool helps you keep the tables and information related to this plugin up to date.', 'wp-notif-bell'); ?>
            </p>
            <?php _e('Last version', 'wp-notif-bell'); ?>: <b class="wpnb-txt-primary"><?php echo Db::LATEST_VERSION; ?></b> <br />
            <?php _e('Your version', 'wp-notif-bell'); ?>: <b><?php echo Db::get_version(); ?></b> <br />

            <?php if (Db::is_last_version()): ?>
                <span class="wpnb-txt-success"><?php _e('The database is up to date.', 'wp-notif-bell'); ?></span>
            <?php else: ?>
                <span class="wpnb-txt-danger"><?php _e('The database needs to be updated.', 'wp-notif-bell'); ?></span>
                <a href="<?php echo add_query_arg([ 'wpnb-process' => 'up-db', 'nonce' => wp_create_nonce('prc:up-db') ]); ?>"><?php _e('Update', 'wp-notif-bell'); ?></a>
            <?php endif; ?>
        </div>
        <?php else: ?>
        <div class="tab-content wpnb-ad-l-box wpnb-mx-0">
            <?php _e('The desired tab does not exist.', 'wp-notif-bell'); ?>
        </div>
        <?php endif; ?>
    </div>
